...
def plot_effective_conductivity(
    k_f_values, k_s, epsilon, d_p, T, P, M_g, C_p, epsilon_r=0.8, phi=0.0077
):
    """
    绘制不同流体热导率下的有效热导率
    """
    ...
    plt.plot(k_f_values, k_eff_values, "b-", linewidth=2, label="k_eff (有效热导率)")
    plt.plot(k_f_values, k_f_values, "r--", linewidth=1.5, label="k_f (流体热导率)")
    plt.plot(
        k_f_values,
        [k_s] * len(k_f_values),
        "g--",
    ...
    plt.xlabel("流体热导率 k_f (W/m·K)")
    plt.ylabel("热导率 (W/m·K)")
    plt.title("Zehner-Bauer-Schlünder模型: 有效热导率 vs 流体热导率")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    plot_effective_conductivity(k_f_range, k_s, epsilon, d_p, T, P, M_g, C_p)
