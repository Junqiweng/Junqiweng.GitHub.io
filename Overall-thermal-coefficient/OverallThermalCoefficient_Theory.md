...
> **⚠️ 计算范围说明**：本模块计算器输出的 $U$ 是**床层内侧等效传热系数**，综合了壁面传热系数 $h_w$ 和有效径向导热系数 $k_{er}$（即下文所说的"床层侧热阻"部分）。**不包含**管壁导热热阻和外侧对流热阻。如需计算完整换热器的 $U_{overall}$，请在本模块结果基础上按串联热阻公式（见本节）叠加管壁和外侧热阻。
